...
import { Loader2, ArrowUpDown, ArrowDown } from "lucide-react"; // Added ArrowDown
...
const CommentSection = ({ discussionId }: { discussionId: string }) => {
  ...
  const fetchComments = useCallback(async (isLoadMore = false) => {
    try {
      ...
      params.set("limit", "10"); // Load 10 at a time
  ...
  }, [discussionId, sort, page]); // Depend on page state
  ...
  return (
    <div className="w-full space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-[#3e2723] pb-4">
        <h3 className="text-lg font-bold text-[#eaddcf]">
           {totalCount} Comments
        </h3>
        ...
        <Select value={sort} onValueChange={(val: "top" | "newest") => setSort(val)}>
            ...
            <SelectContent className="bg-[#1a110d] border-[#3e2723] text-[#eaddcf] z-100" position="popper" align="end">
               <SelectItem value="newest" className="focus:bg-[#3e2723]/40 focus:text-[#d4af37] cursor-pointer">Newest</SelectItem>
               <SelectItem value="top" className="focus:bg-[#3e2723]/40 focus:text-[#d4af37] cursor-pointer">Top Rated</SelectItem>
            </SelectContent>
         </Select>
      </div>
