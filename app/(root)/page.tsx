...
// Lazy Load the AboutHero component
...
const CatSilhouette = ({ className, poseIndex = 0 }: { className?: string; poseIndex?: number }) => (
...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden font-sans bg-[#0f0b0a] text-[#eaddcf]">
      ...
      >
        <motion.div className="relative h-full w-1/2 flex items-center justify-end border-r border-[#1a110d]" style={{ backgroundColor: "#2c1810", backgroundImage: `repeating-linear-gradient(90deg, transparent, transparent 50px, rgba(0,0,0,0.2) 50px, rgba(0,0,0,0.2) 53px), linear-gradient(to bottom, rgba(0,0,0,0.6), rgba(0,0,0,0) 20%, rgba(0,0,0,0) 80%, rgba(0,0,0,0.6))` }} custom="left" variants={doorVariants}>
        ...
        <motion.div className="relative h-full w-1/2 flex items-center justify-start border-l border-[#1a110d]" style={{ backgroundColor: "#2c1810", backgroundImage: `repeating-linear-gradient(90deg, transparent, transparent 50px, rgba(0,0,0,0.2) 50px, rgba(0,0,0,0.2) 53px), linear-gradient(to bottom, rgba(0,0,0,0.6), rgba(0,0,0,0) 20%, rgba(0,0,0,0) 80%, rgba(0,0,0,0.6))` }} custom="right" variants={doorVariants}>
          <div className="absolute inset-0 border-16 border-[#3e2723] opacity-100 box-border pointer-events-none"></div>
          <div className="absolute inset-0 bg-[linear-gradient(transparent_95%,rgba(0,0,0,0.3)_96%)] bg-size-[100%_100px]"></div>
      ...
      {/* ==========================================
          SECTION 1: MAIN LANDING (The Entrance)
      ========================================== */}
      <main className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[#120c0a]">
          ...
          <div className="absolute inset-0 opacity-20" style={{ backgroundImage: `linear-gradient(to right, rgba(255,255,255,0.05) 1px, transparent 1px), linear-gradient(to bottom, rgba(0,0,0,1) 0%, transparent 40%, rgba(0,0,0,1) 100%)`, backgroundSize: "60px 100%" }}></div>
        </div>
          ...
          {backgroundElements.map((el, i) => {
            ...
            return (
              <motion.div
                ...
                transition={{ duration: el.duration, repeat: Infinity, repeatType: "reverse", ease: "easeInOut", delay: el.delay }}
              >
        ...
        {/* --- Main Content --- */}
        <motion.div className="z-10 text-center px-6 relative max-w-7xl w-full" initial="hidden" animate={isDoorOpen ? "visible" : "hidden"} variants={contentVariants}>
          <div className="mb-4">
            <span className="text-[#d4af37] text-sm tracking-[0.5em] opacity-80 font-serif">アルゴリズムを極める</span>
          </div>
          <div className="relative group inline-block">
            <h1 className="text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter leading-none text-[#eaddcf] mix-blend-screen drop-shadow-[0_0_15px_rgba(234,221,207,0.1)] whitespace-nowrap">NEKO<span className="text-[#d4af37] drop-shadow-[0_2px_10px_rgba(212,175,55,0.4)]">DOJO</span></h1>
            <h1 className="absolute inset-0 text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter leading-none text-[#d4af37] opacity-0 group-hover:opacity-10 group-hover:translate-x-1 group-hover:translate-y-1 transition-all duration-300 pointer-events-none select-none whitespace-nowrap">NEKO<span className="">DOJO</span></h1>
          </div>
          ...
          <p className="text-lg md:text-2xl font-light text-[#a1887f] max-w-3xl mx-auto leading-relaxed">Enhance your <span className="text-[#eaddcf] font-medium border-b border-[#d4af37]/30">Data Structures & Algorithms</span> skills<br className="hidden md:block" />in the sanctuary of code.</p>
          <div className="mt-12 flex flex-col md:flex-row gap-8 justify-center items-center">
            <Link href="/sign-in" className="relative px-12 py-4 bg-[#2c1810] border border-[#5d4037] text-[#d4af37] tracking-[0.2em] font-bold uppercase hover:bg-[#3e2723] hover:border-[#d4af37] hover:shadow-[0_0_20px_rgba(212,175,55,0.2)] transition-all duration-500 clip-path-slant">Enter Dojo</Link>
          </div>
        </motion.div>

        {/* Vertical Japanese Side Text */}
        <div className="absolute left-6 bottom-0 hidden lg:block pb-12 z-10">
          <div className="writing-vertical-rl text-[#3e2723] text-opacity-40 text-sm tracking-[0.8em] font-mono h-48 border-l border-[#3e2723]/20 pl-4">継続は力なり</div>
        </div>
        
        <div className="absolute bottom-6 w-full text-center z-10">
          <p className="text-[#3e2723] text-xs tracking-[0.4em] font-mono opacity-50">EST. 2025 // AENANSH</p>
        </div>
        ...
        <motion.div 
         ...
         {/* === THE TRANSITION MIST === 
             This gradient sits at the very bottom of the first section.
             It fades from Transparent -> Black (Active Background Color) -> Black.
             It creates a "Fog" that hides the hard cut between sections. */}
      ...
      {/* ==========================================
          SECTION 2: ABOUT HERO (Lazy Loaded)
      ========================================== */}
      <AboutHero />
