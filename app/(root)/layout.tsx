  ...
  return (
    <>
      <div className="min-h-screen bg-[#0f0b0a] relative selection:bg-[#d4af37] selection:text-[#1a110d]">
        <header className="fixed top-6 left-0 right-0 mx-auto w-[95%] max-w-7xl h-20 z-10 rounded-2xl border border-[#3e2723] bg-[#0f0b0a]/80 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300">
          <div className="w-full h-full px-6 md:px-8 flex items-center justify-between">
            
            {/* === LEFT: BRANDING === */}
            <div className="shrink-0">
              <Link href="/" className="group flex items-center gap-2">
                {/* Optional: Small Icon or Kanji could go here */}
                <span className="font-black text-xl tracking-tighter text-[#eaddcf] select-none">
                  NEKO<span className="text-[#d4af37] group-hover:text-white transition-colors duration-300">DOJO</span>
                </span>
                {/* Subtle Tagline for large screens */}
            ...
            {/* === CENTER: FUTURE NAVIGATION PLACEHOLDER === */}
            ...
            <div className="shrink-0 flex items-center gap-4">
              ...
              <SignedIn>
                {/* User Profile Container (Clean - No Border) */}
                <div className="flex items-center gap-4">
                  <UserButton 
                    appearance={{
                      elements: {
                        avatarBox: "w-8 h-8 border border-[#d4af37]/50 ring-2 ring-transparent hover:ring-[#d4af37]/20 transition-all",
